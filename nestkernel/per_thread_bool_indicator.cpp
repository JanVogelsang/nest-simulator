...
{
  DETAILED_TIMER( kernel().simulation_manager.get_idle_stopwatch(), kernel().vp_manager.get_thread_id() ).start();
...
#pragma omp barrier
  ...
  DETAILED_TIMER( kernel().simulation_manager.get_idle_stopwatch(), kernel().vp_manager.get_thread_id() ).stop();
  return ret;
}
