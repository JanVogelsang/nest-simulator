...
nest::SourceTable::initialize()
{
  ...
  compressed_spike_data_.resize( num_synapse_models );
  compressed_spike_data_map_.resize( num_synapse_models, std::map< size_t, size_t >() );
...
nest::SourceTable::remove_disabled_sources( const size_t tid, const synindex syn_id )
{
  ...
  {
    return invalid_lcid; // no source table entry for this synapse model
  }
  ...
  if ( max_size == 0 )
  {
    return invalid_lcid; // no connections for this synapse model
  }
  ...
  {
    return invalid_lcid; // all lcids are valid, nothing to remove
  }
...
nest::SourceTable::resize_sources()
{
  ...
  sources_.at( kernel().vp_manager.get_thread_id() ).resize( num_connection_models );
  compressed_spike_data_.resize( num_connection_models );
  ...
  const Source& source ) const
{
  ...
  return not( // source.is_processed() or
    source.is_disabled()
...
{
  assert( not kernel().vp_manager.use_compressed_spikes() ); // handled elsewhere
  ...
  // TODO JV: Check if primary bit of Source can be removed
...
void
nest::SourceTable::fill_compressed_spike_data( const size_t tid )
{
  // For each synapse type, and for each source neuron with at least one local target,
  // store in compressed_spike_data the first connection index from that source for each local thread that
  // owns a local target, as well as the number of following connections from the same source. In
  // compressed_spike_data_map_ store index into compressed_spike_data[syn_id] where data for a given source is stored.

  for ( synindex syn_id = 0; syn_id < kernel().model_manager.get_num_connection_models(); ++syn_id )
  {
    for ( size_t source_lcid = 0; source_lcid != sources_[ tid ][ syn_id ].size(); )
    {
      const size_t source_gid = sources_[ tid ][ syn_id ][ source_lcid ].get_node_id();
      auto it = compressed_spike_data_map_[ syn_id ].find( source_gid );
      assert( it != compressed_spike_data_map_[ syn_id ].end() );
      compressed_spike_data_[ syn_id ][ it->second ][ tid ].set_lcid( source_lcid );
      source_lcid += compressed_spike_data_[ syn_id ][ it->second ][ tid ].get_num_connections();
      assert( source_gid != sources_[ tid ][ syn_id ][ source_lcid ].get_node_id() );
    }
  }
}
...
nest::SourceTable::dump_compressed_spike_data(
  const std::vector< std::vector< std::vector< CompressedSpikeData > > >& FULL_LOGGING_ONLY(
    compressed_spike_data ) ) const
{
  FULL_LOGGING_ONLY(
    for ( const auto& tab
