...
public:
  ...
  /**
   * Process a post-synaptic spike after it backpropagated to the synapse.
  ...
  check_connection( Node& s, Node& t, const rport receptor_type, const synindex syn_id, const CommonPropertiesType& cp )
  {
    ...
    if ( get_axonal_delay() > ( delay - get_axonal_delay() ) )
    {
      LOG( M_WARNING,
    ...
    t.register_stdp_connection( t_lastspike_ - delay + 2.0 * get_axonal_delay(), delay );
  }
 ...
 */
inline void
stdp_pl_synapse_hom_ax_delay::send( Event& e, thread _, const STDPPLHomAxDelayCommonProperties& cp, Node* target )
{
...
stdp_pl_synapse_hom_ax_delay::process_post_synaptic_spike( double t_syn, const STDPPLHomAxDelayCommonProperties& cp )
{
  ...
  double minus_dt = t_lastspike_ + axonal_delay - t_syn;
