...
public:
   ...
   */
  void send( Event& e, const thread t, const delay dendritic_delay, const STDPHomCommonProperties&, Node* target );
  ...
  Node* target )
{
  ...
  weight_ =
    depress_( weight_, target->get_K_value( dendritic_delay_ms, t_spike, e.get_sender_spike_data().get_syn_id() ), cp );
